...
def main():
    if len(sys.argv) < 2:
        print("用法: python scripts/delete_puzzles_full_cascade.py <title1> [title2] [title3] ...")
        sys.exit(1)
    ...
    try:
        with conn.cursor() as cur:
            ...
            # 查询所有相关记录
            if puzzle_ids:
                id_placeholders = ','.join(['%s'] * len(puzzle_ids))
                
                # 存档记录
                cur.execute(
                    f"SELECT COUNT(*) AS cnt FROM puzzle_progress WHERE puzzle_id IN ({id_placeholders})",
                    puzzle_ids
                )
                ...
                # 排行榜记录
                cur.execute(
                    f"SELECT COUNT(*) AS cnt FROM rank_records WHERE level_id IN ({id_placeholders})",
                    puzzle_ids
                )
                rank_count = cur.fetchone()["cnt"]
                
                print(f"相关存档记录数: {progress_count}")
                print(f"相关排行榜记录数: {rank_count}")
            
            # 确认删除
            confirm = input("确认删除这些关卡及所有相关记录吗？(y/N): ")
            if confirm.lower() != 'y':
                print("取消删除操作。")
                return
            ...
            if puzzle_ids:
                ...
                if progress_count > 0:
                    print("删除存档记录...")
                    cur.execute(
                        f"DELETE FROM puzzle_progress WHERE puzzle_id IN ({id_placeholders})",
                        puzzle_ids
                    )
                    print(f"删除存档记录数: {cur.rowcount}")
                
                # 2. 删除排行榜记录
                if rank_count > 0:
                    print("删除排行榜记录...")
                    cur.execute(
                        f"DELETE FROM rank_records WHERE level_id IN ({id_placeholders})",
                        puzzle_ids
                    )
                    print(f"删除排行榜记录数: {cur.rowcount}")
