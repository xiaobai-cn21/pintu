...
def get_ranking_by_level(level_id):
    """根据关卡ID获取该关卡的排名数据"""
    try:
        # 不再检查关卡是否存在于puzzles表中，允许任何有效的关卡ID查询排名
        # 这样新创建的关卡也能正确显示排名信息
...
def get_level_info(level_id):
    ...
    try:
        ...
        if puzzle:
            # 如果关卡存在于puzzles表中，返回实际信息
            level_info = {
                'level_id': puzzle.puzzle_id,
                'level_name': puzzle.title,
                'difficulty': puzzle.difficulty,
                'piece_count': puzzle.piece_count
            }
        else:
            # 如果关卡不存在于puzzles表中（如用户自定义关卡），返回默认信息
            # 这样可以确保前端始终能获取到有效的关卡信息
            level_info = {
                'level_id': level_id,
                'level_name': f'自定义关卡{level_id}',
                'difficulty': 'custom',
                'piece_count': 9  # 默认9块拼图
            }
